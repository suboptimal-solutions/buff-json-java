...
class Proto3JsonDeserializationConformanceTest {

	private static final JsonFormat.Printer PRINTER = JsonFormat.printer().omittingInsignificantWhitespace();
	private static final Decoder CODEGEN_DECODER = BuffJSON.decoder().withGeneratedDecoders(true);
	private static final Decoder GENERIC_DECODER = BuffJSON.decoder().withGeneratedDecoders(false);
	...
	private <T extends Message> void assertDecodeMatchesOriginal(T original) throws Exception {
		...
		// Test generic path
		T generic = GENERIC_DECODER.decode(json, clazz);
		assertEquals(original, generic, "Generic mismatch for " + typeName + " json=" + json);
	}
	...
	class AnyTests {
		...
		private static final Decoder DECODER = BuffJSON.decoder().withTypeRegistry(TYPE_REGISTRY);
		private static final Decoder GENERIC_ANY_DECODER = DECODER.withGeneratedDecoders(false);
		...
		private <T extends Message> void assertAnyDecodeMatchesOriginal(T original) throws Exception {
			...
			T generic = GENERIC_ANY_DECODER.decode(json, clazz);
			assertEquals(original, generic, "Generic mismatch for " + typeName + " json=" + json);
		}
		...
		void roundTripAnyContainingRegularMessage() {
			...
			Encoder encoder = BuffJSON.encoder().withTypeRegistry(TYPE_REGISTRY);
			...
			TestAny generic = GENERIC_ANY_DECODER.decode(json, TestAny.class);
			assertEquals(original, generic, "Round-trip generic mismatch, json=" + json);
		}
	...
	class RoundTrip {
		...
		private <T extends Message> void assertRoundTrip(T original) {
			...
			// generic encode → generic decode
			String genericJson = GENERIC_ENCODER.encode(original);
			T genericResult = GENERIC_DECODER.decode(genericJson, clazz);
			assertEquals(original, genericResult,
					"Generic round-trip for " + original.getDescriptorForType().getFullName());

			// cross: codegen encode → generic decode
			T crossResult = GENERIC_DECODER.decode(codegenJson, clazz);
