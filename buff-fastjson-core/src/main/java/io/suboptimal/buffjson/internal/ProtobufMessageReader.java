...
public final class ProtobufMessageReader implements ObjectReader<Message> {
	...
	public Message readObject(JSONReader reader, Type fieldType, Object fieldName, long features) {
		...
		return readMessageGeneric(reader, descriptor, defaultInstance);
	}

	/**
	 * Reads a message using the generic reflection-based path. Used internally for
	...
	private static Message tryGeneratedDecode(JSONReader reader, Descriptor descriptor) {
		if (GeneratedDecoderRegistry.hasDecoders() && Boolean.TRUE != BuffJSON.SKIP_GENERATED_DECODERS.get()) {
			GeneratedDecoder<Message> decoder = GeneratedDecoderRegistry.get(descriptor);
			if (decoder != null) {
	...
	/**
	 * Generic reflection-based message reading. Always uses the descriptor/builder
	 * path.
	 */
	static Message readMessageGeneric(JSONReader reader, Descriptor descriptor, Message defaultInstance) {
